  ...
  // ✅ COMPONENTE SIMPLE Y ROBUSTO
  const VideoDisplay = ({ onCameraSwitch, mainCamera }) => {
    const participants = useParticipants();
    const localParticipant = participants.find(p => p.isLocal);
    const remoteParticipant = participants.find(p => !p.isLocal);

    // Obtener tracks de video de forma MUY segura
    const getLocalVideoTrack = () => {
      if (!localParticipant?.videoTracks) return null;
      try {
    ...
    const getRemoteVideoTrack = () => {
      if (!remoteParticipant?.videoTracks) return null;
      try {
    ...
    const remoteVideoTrack = getRemoteVideoTrack();

    const getMainVideo = () => {
      try {
        if (mainCamera === "local" && localVideoTrack && localParticipant?.videoTracks) {
          const publication = Array.from(localParticipant.videoTracks.values())[0];
          if (publication) {
            return <VideoTrack trackRef={{ participant: localParticipant, publication, source: Track.Source.Camera }} className="w-full h-full object-cover" />;
          }
        } else if (remoteVideoTrack && remoteParticipant?.videoTracks) {
          const publication = Array.from(remoteParticipant.videoTracks.values())[0];
          if (publication) {
            return <VideoTrack trackRef={{ participant: remoteParticipant, publication, source: Track.Source.Camera }} className="w-full h-full object-cover" />;
          }
        }
      } catch (error) {
        ...
        <div className="w-full h-full flex items-center justify-center bg-gray-800 text-white">
          <div className="text-center">
            ...
            <p className="text-sm text-gray-400 mt-2">
              {participants.length > 0 ? `${participants.length} participante(s)` : 'Conectando...'}
            </p>
          </div>
        </div>
      );
    ...
    const getMiniVideo = () => {
      try {
        if (mainCamera === "local" && remoteVideoTrack && remoteParticipant?.videoTracks) {
          const publication = Array.from(remoteParticipant.videoTracks.values())[0];
          if (publication) {
            return <VideoTrack trackRef={{ participant: remoteParticipant, publication, source: Track.Source.Camera }} className="w-full h-full object-cover" />;
          }
        } else if (localVideoTrack && localParticipant?.videoTracks) {
          const publication = Array.from(localParticipant.videoTracks.values())[0];
          if (publication) {
            return <VideoTrack trackRef={{ participant: localParticipant, publication, source: Track.Source.Camera }} className="w-full h-full object-cover" />;
          }
    ...
    useEffect(() => {
      const getToken = async () => {
        try {
          ...
          console.log("Obteniendo token para:", { roomName, userName });
          ...
          console.error('Error completo al obtener token:', err);
          ...
          <div className="text-center">
            ...
            {modelo && (
              <div className="mt-4 text-sm text-gray-400">
                <p>Conectando con: {modelo.nombre} {modelo.pais}</p>
              </div>
            )}
          </div>
        </div>
          ...
          <div className="w-[340px] bg-[#1f2125] rounded-2xl flex flex-col justify-between relative">
            ...
            <div className="flex-1 p-4 space-y-3 overflow-y-auto custom-scroll">
              ...
              <div className="text-right">
                <p className="bg-[#ff007a] inline-block px-3 py-1 mt-1 rounded-xl text-white">¡Bien, gracias!</p>
              </div>
            </div>
            ...
            {mostrarRegalos && (
              <div className="absolute bottom-[70px] left-1/2 transform -translate-x-1/2 bg-[#1a1c20] p-5 rounded-xl shadow-2xl w-[300px] max-h-[360px] overflow-y-auto z-50 border border-[#ff007a]/30 scrollbar-thin scrollbar-thumb-[#ff007a88] scrollbar-track-[#2b2d31]">
                <div className="flex justify-between items-center mb-3">
                  <h3 className="text-white font-semibold text-sm">🎁 Elige un regalo</h3>
